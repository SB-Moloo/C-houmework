...
int InputRecord(STUDENT student[31], int *point, int coursenumb)
{
	...
	if(mode)
	{
		if((fp=fopen("sourcedata.txt","r"))==NULL)
		{
			printf("Failure to open sourcedata.txt!\n");
		}
		else
		{
			for( ; i<30 ; i++)
			{
				if(student[i-1].studentnumb != -1)
				{
					fscanf(fp,"%ld",&student[i].studentnumb);
					fscanf(fp,"%s",&student[i].name);
					for(m=0 ; m<coursenumb ; m++)
					{
						fscanf(fp,"%lf",&student[i].score[m]);
					}
					k = i-1;
					*point = k;
				}
...
int ListRecord(STUDENT student[31], COURSE course[7], int coursenumb, int k)
{
	...
	else
	{
		for(i=0;i<=k;i++)
		{
			fprintf(fp,"%-20ld%-20s",student[i].studentnumb,student[i].name);
			for(tool=0;tool<coursenumb;tool++)
			{
				fprintf(fp,"%-10.2f",student[i].score[tool]);
			}
			fprintf(fp,"\n");
		}
	}
